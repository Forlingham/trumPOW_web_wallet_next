...
const translations = {
  en: {
    ...
    'transactions.recent': 'Recent Txs',
    ...
    'send.errorInfo': 'Please check if the wallet is connected to the network.',
    ...
    'common.supportAuthorDesc':
      'Your donations not only support the developer but also serve as a driving force for new features, contributing to the growth of the TRMP community.',
  ...
  <ul>
    ...
    <li>After signing, only the raw transaction (RawTx) is broadcast to the TRMP network.</li>
  </ul>
  ...
  <ul>
    <li>Implemented with <strong>Nest.js</strong>, handling secure communication with TRMP nodes.</li>
  ...
  <ul>
    <li>Integrates with TRMP via <strong>RPC</strong>, supporting transaction queries, balance checks, and broadcasting.</li>
  ...
  <blockquote>With this architecture, we aim to deliver a secure, transparent, and user-friendly wallet for the TRMP community.</blockquote>
</section>
     `
  },
  zh: {
    // Wallet Setup
    'wallet.title': 'TRMP 钱包',
    ...
    'common.supportAuthorDesc': '大家的捐赠不仅是对作者的支持，更是推动新功能开发的动力，为 TRMP 社区的发展贡献一份力量。',
  ...
  <ul>
    ...
    <li>签名完成后，仅会将原始交易（RawTx）广播到 TRMP 网络。</li>
  </ul>
  ...
  <h3>2. 后端服务</h3>
  <ul>
    <li>基于 <strong>Nest.js</strong> 框架开发，负责与 TRMP 节点进行安全通讯。</li>
  ...
  <h3>3. 区块链交互</h3>
  <ul>
    <li>通过 <strong>TRMP RPC</strong> 与区块链节点交互，支持交易查询、余额获取和广播。</li>
  ...
  <blockquote>我们希望通过这一技术架构，为 TRMP 社区提供一个安全、透明且易用的钱包解决方案。</blockquote>
</section>
    `
  },
  ru: {
    // Wallet Setup
    'wallet.title': 'TRMP Кошелек',
    ...
    'send.errorInfo': 'Пожалуйста, обновите кошелек и повторите попытку.',
    ...
    'common.supportAuthorDesc':
      'Ваши пожертвования не только поддерживают разработчика, но и становятся движущей силой для создания новых функций, внося вклад в развитие сообщества TRMP.',
  ...
  <ul>
    ...
    <li>После подписи в сеть TRMP передаётся только готовая транзакция (RawTx).</li>
  </ul>
  ...
  <ul>
    <li>Реализован с использованием <strong>Nest.js</strong>, обеспечивает безопасное взаимодействие с узлами TRMP.</li>
  ...
  <ul>
    <li>Интеграция через <strong>RPC</strong> TRMP: поддерживаются запросы транзакций, проверка баланса и трансляция транзакций.</li>
  ...
  <blockquote>Мы стремимся предоставить сообществу TRMP безопасный, прозрачный и удобный кошелёк на основе этой архитектуры.</blockquote>
</section>
     `
  }
}
