...
export function getRawTransactionApi(txid: string, debounceMs: number = 300): Promise<ApiData<RpcRes<RawTransaction>>> {
  const key = `getrawtransaction_${txid}`
  return new Promise((resolve, reject) => {
    // 清除之前的防抖定时器
    if (debounceTimers.has(key)) {
      clearTimeout(debounceTimers.get(key)!)
 ...
 * @returns 交易详情
 */
export function getTransactionApi(txid: string, debounceMs: number = 300): Promise<ApiData<RpcRes<string>>> {
  const key = `gettransaction_${txid}`
  return new Promise((resolve, reject) => {
    // 清除之前的防抖定时器
    if (debounceTimers.has(key)) {
      clearTimeout(debounceTimers.get(key)!)
    ...
    const timer = setTimeout(async () => {
      try {
        // 创建实际的API请求
        const result = await axiosTool.post<RpcRes<string>>('/rpc/gettransaction', {
          txid
        })
